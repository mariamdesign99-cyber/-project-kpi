...
import type { Kpi, ChatMessage } from '../App.tsx';
import type { AnalysisData } from './KpiCard.tsx';
import { Loader } from './Loader.tsx';
import { generateChatResponseStream } from '../services/geminiService.ts';
...
export const AiChatModal: React.FC<AiChatModalProps> = ({ isOpen, onClose, kpi, initialAnalysisData, isLoading, error, chatHistory, onChatUpdate }) => {
    ...
    useEffect(() => {
        if(isOpen) {
            ...
            setIsCopied(false);
            setIsThinking(false);
            stopGenerationRef.current = true; // Stop any pending generation on open
        } else {
            // Also stop generation and clear streaming state on close
