...
export const KpiCorrelation: React.FC<KpiCorrelationProps> = ({ kpis }) => {
    ...
    const debouncedKpi1Id = useDebounce(kpi1Id, 500);
    const debouncedKpi2Id = useDebounce(kpi2Id, 500);
    ...
    const debouncedKpi1 = useMemo(() => validKpis.find(k => k.id === debouncedKpi1Id), [validKpis, debouncedKpi1Id]);
    const debouncedKpi2 = useMemo(() => validKpis.find(k => k.id === debouncedKpi2Id), [validKpis, debouncedKpi2Id]);
    ...
    useEffect(() => {
        ...
        if (debouncedKpi1 && debouncedKpi2 && debouncedKpi1.id !== debouncedKpi2.id) {
            const fetchAnalysis = async () => {
                setIsLoading(true);
                setAnalysis(null);
                const currentCorrelation = calculateCorrelation(debouncedKpi1.history!, debouncedKpi2.history!);
                try {
                    const result = await generateCorrelationAnalysis(debouncedKpi1.title, debouncedKpi2.title, currentCorrelation);
                    setAnalysis(result);
                } catch (error) {
            ...
            fetchAnalysis();
             ...
             setIsLoading(false);
             setAnalysis(debouncedKpi1?.id === debouncedKpi2?.id ? "Показатель всегда идеально коррелирует сам с собой." : "Выберите два разных показателя для анализа.");
        }
    }, [debouncedKpi1, debouncedKpi2]);
