...
const datesAreEqual = (d1: Date | null, d2: Date | null): boolean => {
...
export const DateRangePicker: React.FC<DateRangePickerProps> = ({ onChange, initialRange }) => {
    ...
    // This effect determines the active preset based on the current range.
    useEffect(() => {
        const today = getToday();
        if (datesAreEqual(range.end, today)) {
            if (datesAreEqual(range.start, today)) {
                setActivePreset('day'); return;
            }
            if (datesAreEqual(range.start, subDays(today, 6))) {
                setActivePreset('week'); return;
            }
            if (datesAreEqual(range.start, subMonths(today, 1))) {
                setActivePreset('month'); return;
            }
             if (datesAreEqual(range.start, subMonths(today, 3))) {
                setActivePreset('quarter'); return;
            }
            if (datesAreEqual(range.start, subYears(today, 1))) {
                setActivePreset('year'); return;
            }
        }
    ...
    const handlePresetClick = (preset: Preset) => {
        ...
        // No need to setActivePreset here, the useEffect will handle it.
        setIsOpen(false);
    };

    const handleDateSelect = (date: Date) => {
        // This will automatically set the preset to 'custom' via the useEffect
    ...
    return (
        <div className="relative" ref={wrapperRef}>
            <div className="flex flex-wrap items-center gap-1 p-1 bg-gray-100 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg">
                ...
                <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-xs sm:text-sm font-semibold transition-colors ${activePreset === 'custom' ? 'bg-white dark:bg-gray-700 text-gray-800 dark:text-white shadow-sm' : 'bg-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700/50'}`}>
