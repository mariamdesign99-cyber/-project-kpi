...
import { calculateKpis, BaseMetrics } from '../services/kpiService.ts';
import { CalculatorIcon, TrendingUpIcon, TrendingDownIcon } from './icons.tsx';
...
export const KpiCalculator: React.FC = () => {
  ...
  const getChangeIndicator = (key: string, base: number, alternative: number) => {
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {results.map(({ key, name, baseValue, alternativeValue }) => (
                            <tr key={key}>
                                ...
                                <td className="py-3 px-4 text-right">{getChangeIndicator(key, baseValue, alternativeValue)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
