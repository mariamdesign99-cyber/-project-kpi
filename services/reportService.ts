...
export function generateReportData(reportType: string, dateRange: { start: Date, end: Date }): ReportData {
    const kpiMap: Record<string, { title: string; icon: string }> = {
        revenue: { title: "Выручка", icon: "DollarSignIcon" },
        sales: { title: "Продажи", icon: "ShoppingCartIcon" },
        new_clients: { title: "Новые клиенты", icon: "UsersIcon" },
        conversion: { title: "Конверсия", icon: "MousePointerClickIcon" },
        employee_turnover: { title: "Текучесть кадров", icon: "UsersIcon" },
        ticket_resolution_time: { title: "Время решения тикета", icon: "ClipboardListIcon" },
    };
    ...
    const generatedKpis = config.kpiIds.map(id => {
        ...
        return {
            ...
            icon: kpiMap[id]?.icon || 'BriefcaseIcon',
        };
    });
