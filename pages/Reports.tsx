...
import { 
    DownloadIcon, FilePieChartIcon, SparklesIcon, CalendarIcon, BriefcaseIcon, BotIcon, SendIcon,
    DollarSignIcon, ShoppingCartIcon, UsersIcon, MousePointerClickIcon, ClipboardListIcon
} from '../components/icons.tsx';
import { DateRangePicker, type DateRange } from '../components/DateRangePicker.tsx';
import { Loader } from '../components/Loader.tsx';
import { MainKpiChart } from '../components/MainKpiChart.tsx';
import { generateReportData, generatePdf, generateCsv, type ReportData } from '../services/reportService.ts';
import { generateReportAnalysis, generateReportChatResponseStream } from '../services/geminiService.ts';
import { AnalysisActions } from '../components/AnalysisActions.tsx';
...
const ReportMetricCard: React.FC<{ metric: ReportData['kpis'][0] }> = ({ metric }) => {
    ...
    const IconComponent = iconComponents[metric.icon] || BriefcaseIcon;
    return (
...
const DownloadButton: React.FC<{ onDownload: (format: 'pdf' | 'csv' | 'excel') => void, disabled: boolean, isLoading: boolean }> = ({ onDownload, disabled, isLoading }) => {
    ...
    return (
        <div ref={menuRef} className="relative">
            ...
            {isOpen && (
                <div role="menu" className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-10 animate-fade-in">
                    <button role="menuitem" onClick={() => handleSelect('pdf')} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">PDF-документ</button>
                    <button role="menuitem" onClick={() => handleSelect('excel')} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">Excel (.csv)</button>
                    <button role="menuitem" onClick={() => handleSelect('csv')} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">CSV-файл</button>
                </div>
            )}
        </div>
    );
